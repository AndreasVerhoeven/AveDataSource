...
public enum List {
	...
	public struct Result<ItemType: Identifiable> {
		...
		public var common = IndexSet()
		public var moves = Array<MoveIndex>()
		public var oldMap = Dictionary<ItemType.ID, Int>()
		public var newMap = Dictionary<ItemType.ID, Int>()
		...
		public func validate(_ oldArray: Array<ItemType>, _ newArray: Array<ItemType>) -> Bool {
			return (oldArray.count + self.inserts.count - self.deletes.count) == newArray.count
		}
		public func oldIndexFor(identifier: ItemType.ID) -> Int? {
			return self.oldMap[identifier]
		}
		public func newIndexFor(identifier: ItemType.ID) -> Int? {
	...
	public static func diffing<T: Identifiable>(oldArray:Array<T>, newArray:Array<T>) -> Result<T> {
		...
		var table = Dictionary<T.ID, Entry>()
...
public extension List.Result {
	func forBatchUpdates() -> List.Result<ItemType> {
		var result = self
